...
if (!$eventDetails || $eventDetails['creator_id'] !== $_SESSION['user_id']) {
    header('Location: events.php');
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $old = $eventDetails;
    $old['event_date'] = date('Y-m-d\TH:i', strtotime($eventDetails['event_date']));
}
?>
...
<main class="text-center h-full flex-1 flex flex-col gap-8 items-center">
    <form action="" method="post" class="flex flex-col mt-12 w-1/2 py-12 px-10 bg-neutral-900 rounded-2xl border border-red-600 shadow-xl shadow-red-600/50">
        ...
        <button type="submit" class="mt-8 w-full bg-red-600 text-white font-bold py-2 rounded-lg hover:bg-red-500">Update Event</button>
    </form>
</main>
